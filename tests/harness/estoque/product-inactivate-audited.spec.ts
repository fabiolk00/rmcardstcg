...
/**
 * FEATURE: estoque.product.inactivate-audited (priority 18) — DB-first, sem browser.
 *
 * Prova "admin inativa produto (soft-delete) com auditoria" contra o Postgres
 ...
 * (price-update-discount-audited.spec.ts / product-create.spec.ts): roda em Node
 ...
 * writeAuditLog na MESMA tx, action product.inactivate, before/after }). NAO
 * chamamos a server action setProductActiveAction porque ela comeca com
 ...
 * run), ATIVO. Antes de inativar, FORCAMOS reserved>0 (UPDATE direto na fixture,
 * respeitando o CHECK reserved<=stock) para que o assert "reserved inalterado" NAO
 * vire trivial. Nao tocamos o seed.
 *
 ...
 * processo `tsx` separado (tests/harness/estoque/_run-seam.ts, estendido com o case
...
const P = 23456; // priceCents base (intocado pelo soft-delete)
const DISC = 10; // desconto (intocado)
const STOCK = 12; // estoque (intocado)
const RESERVED = 4; // reserved FORCADO > 0 p/ tornar o assert "reserved inalterado" nao-trivial

test("estoque.product.inactivate-audited: isActive true->false (soft-delete), audita na mesma tx, estado preservado", async () => {
  const client = makeClient();
  await client.connect();
  try {
    const tag = randomUUID().slice(0, 8);
    const name = `Produto Harness Inactivate ${tag}`;
    const sku = `HARNESS-INACT-${tag}`;
    ...
    const created = runProductSeam("createProduct", {
      ...
      input: {
        ...
        description: "fixture do harness para inactivate-audited",
      },
    });
    ...
    //     "reserved inalterado" seria trivial (reserved=0 antes e depois). Respeita
    //     o CHECK products_reserved_le_stock_chk (0 <= RESERVED <= STOCK).
    await client.query(`UPDATE "products" SET reserved = $1 WHERE id = $2`, [RESERVED, productId]);

    // Snapshot pre-inativacao: is_active=true e os campos que NAO podem mudar.
    ...
    const p0 = pre.rows[0];
    expect(p0.is_active, "setup deixa o produto ativo").toBe(true);
    ...
    // Contagens iniciais: total de produtos (p/ provar soft-delete = NENHUMA remocao)
    // e audit_log (total + por entidade) p/ provar "exatamente 1 linha nova".
    ...
    expect(auditEntityBefore, "createProduct ja deixou 1 audit deste produto").toBe(1);

    // --- passo 2: chama setProductActive(actor, id, false).
    ...
    expect(result.isActive, "retorno reflete isActive=false").toBe(false);

    // --- assert [1]: products.isActive == false; NENHUMA linha removida (soft-delete).
    ...
    expect(after.rowCount, "a linha continua existindo (soft-delete, nao hard-delete)").toBe(1);
    const p1 = after.rows[0];
    expect(p1.is_active, "isActive deve passar a false").toBe(false);
    ...
    expect(
      ...
      "soft-delete NAO remove linha: contagem de produtos inalterada",
    ).toBe(productsBefore);

    // --- assert [3]: stock/reserved/preco INALTERADOS (so is_active mudou).
    ...
    expect(p1.reserved, "reserved (>0) inalterado pelo soft-delete").toBe(RESERVED);
    expect(p1.slug, "slug inalterado").toBe(p0.slug);
    expect(p1.sku, "sku inalterado").toBe(p0.sku);

    // --- assert [2]: audit_log recebe EXATAMENTE 1 linha nova, action=product.inactivate,
    //     before.isActive=true, after.isActive=false, na MESMA transacao.
    ...
    // action gravado com o valor DOTTED do enum (schema.prisma usa @map("product.inactivate")).
    expect(a.action, "action deve ser o @map dotted product.inactivate (nao a chave JS)").toBe(
      "product.inactivate",
    );
    expect(a.entity_type).toBe("product");
    expect(a.entity_id).toBe(productId);

    expect(a.before, "inactivate: before deve ser snapshot (nao-null)").toBeTruthy();
    expect(a.after, "inactivate: after deve ser snapshot (nao-null)").toBeTruthy();
    expect(a.before!.isActive, "before.isActive deve ser true").toBe(true);
    expect(a.after!.isActive, "after.isActive deve ser false").toBe(false);
    ...
    //     viola 0<=reserved<=stock (o soft-delete nao corrompeu o invariante de estoque).
