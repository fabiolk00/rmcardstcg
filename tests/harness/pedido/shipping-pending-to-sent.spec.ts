...
/**
 * FEATURE: pedido.shipping.pending-to-sent (priority 7) — DB-first, sem browser.
 *
 * Prova "admin marca envio pending -> sent (auditado)" contra o Postgres efemero
 ...
 * no-op, valida a transicao contra SHIPPING_TRANSITIONS (pending inclui 'sent' =>
 ...
 * shippingStatus=pending + 1 item (QTY>0). ANTI-TRIVIALIDADE p/ "nenhum efeito de
 * estoque": FORCAMOS o produto com reserved=RESERVED0(>0) e stock=STOCK0 e o pedido
 * com stockReserved=true (reserva ativa). Assim, SE a transicao 'sent' tocasse o
 * estoque por engano, stock/reserved/flags mudariam — o teste so passa porque a
 * transicao de envio (nao-cancelamento) NAO concilia: stock/reserved/flags ficam
 * INTACTOS. reserved e gerido pelo ciclo de reserva (reserveStock no checkout),
...
test("pedido.shipping.pending-to-sent: transicao legal auditada na mesma tx, sem efeito de estoque", async () => {
  ...
  try {
    ...
    const created = runSeam<SeamProduct>("createProduct", {
      ...
      input: {
        name: `Produto Harness Ship ${tag}`,
        category: "Booster Box",
        sku: `HARNESS-SHIP-${tag}`,
        priceCents: UNIT_PRICE,
        discountPct: 0,
        stock: 999, // valor inicial irrelevante; forcado abaixo
        badge: null,
        imageUrl: "/products/placeholder.svg",
        description: "fixture do harness para shipping-pending-to-sent",
      },
    });
    const productId = created.id;

    // --- setup B: FORCA reserved=RESERVED0(>0) e stock=STOCK0 p/ refletir uma reserva
    //     ativa. Se 'sent' tocasse estoque por engano, isso mudaria; aqui prova que NAO
    //     muda. reserved e gerido pelo ciclo de reserva (nunca por admin avulso), entao o
    ...
    await client.query(
      `INSERT INTO "order_items" (id, order_id, product_id, product_name, quantity, unit_price_cents)
         VALUES ($1, $2, $3, $4, $5, $6)`,
      [randomUUID(), orderId, productId, `Produto Harness Ship ${tag}`, QTY, UNIT_PRICE],
    );
    ...
    // --- acao: updateOrderShippingStatus(orderId, 'sent', actor) (seam de PRODUCAO).
    const res = runSeam<AdminOrderUpdate>("updateOrderShippingStatus", {
      orderId,
      to: "sent",
      actor,
    });
    expect(res.ok, "transicao pending->sent deve ser ok").toBe(true);
    if (res.ok) {
      expect(res.changed, "transicao legal deve aplicar (changed=true)").toBe(true);
      expect(res.order.shippingStatus, "order retornado em sent").toBe("sent");
    }

    // --- assert 1: orders.shipping_status == 'sent' (transicao legal de SHIPPING_TRANSITIONS).
    ...
    // after.shippingStatus=sent; paymentStatus identico nos dois (delta limpo: so envio mudou).
    ...
    expect(a.after!.shippingStatus, "after.shippingStatus deve refletir 'sent'").toBe("sent");
    expect(
