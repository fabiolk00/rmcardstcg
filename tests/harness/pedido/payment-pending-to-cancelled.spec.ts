...
/**
 * FEATURE: pedido.payment.pending-to-cancelled (priority 2) — DB-first, sem browser.
 *
 * Prova "admin marca pagamento pending -> cancelled (estorna reserva)" contra o
 ...
 * payment_status (updateMany WHERE paymentStatus=from), concilia o estoque via
 * reconcileStockForPaymentStatus (ramo 'cancelled', orders.ts L457-466: CAS
 *   UPDATE orders SET stock_reserved=false WHERE id=? AND stock_reserved=true AND stock_committed=false
 * e — SE released===1 — releaseStock(tx, lines): reserved-=qty, stock INTOCADO) e
 * grava audit_log (action order.payment_status_update, after.manualAdjustment=true /
 * adjustmentReason=reason) — TUDO na MESMA tx. NAO chamamos a server action
 ...
 * a reserva desse pedido (reserved=QTY>0, stock=STOCK0) via UPDATE direto: reserved
 * e gerido pelo ciclo de reserva (reserveStock no checkout), nunca por uma escrita
 * de admin avulsa, entao o seed direto e a forma honesta de montar o pre-estado.
 * Assim o estorno tem efeito REAL e nao-trivial (reserved 3->0) e provamos que o
 * stock fisico NAO muda (10->10), distinguindo release de commit.
 *
 ...
 * adjustOrderPaymentStatus c/ qualquer PaymentStatus); o spec faz TODAS as assercoes via `pg`.
 *
 * Invariantes cobertas: order-state-machine (pending->cancelled legal, CAS),
 * reserve-lifecycle-idempotent (stockReserved vira false via CAS no proprio pedido,
 * release nao baixa estoque fisico), reserved-le-stock (CHECK + 0 violacoes),
...
const STOCK0 = 10; // estoque FISICO do produto (deve permanecer 10: release nao baixa stock)
...
test("pedido.payment.pending-to-cancelled: estorna reserva e audita na mesma tx", async () => {
  const client = makeClient();
  await client.connect();
  try {
    const tag = randomUUID().slice(0, 8);
    const reason = `cancelamento manual harness ${tag}`;
    ...
    const created = runSeam<SeamProduct>("createProduct", {
      ...
      input: {
        ...
        description: "fixture do harness para payment-pending-to-cancelled",
      },
    });
    const productId = created.id;

    // --- setup B: FORCA reserved=RESERVED0(>0) e stock=STOCK0 p/ refletir a reserva
    //     do pedido que vamos criar. reserved e gerido pelo ciclo de reserva (nunca
    //     por escrita de admin avulsa), entao o UPDATE direto e a forma honesta de
    ...
    if (res.ok) {
      expect(res.changed, "transicao pending->cancelled deve aplicar (changed=true)").toBe(true);
      expect(res.order.paymentStatus, "order retornado em cancelled").toBe("cancelled");
    }

    // --- assert 1: orders.payment_status == 'cancelled' (transicao legal pending->cancelled).
    ...
    // --- assert 2: releaseStock aplicado: products.reserved -= QTY; stockReserved=false;
    //     products.stock INALTERADO (a reserva nunca baixou estoque fisico).
    const postP = await client.query<{ stock: number; reserved: number }>(
      `SELECT stock, reserved FROM "products" WHERE id = $1`,
      [productId],
    );
    expect(postP.rowCount).toBe(1);
    expect(postP.rows[0].reserved, "reserved deve baixar QTY (estorno da reserva)").toBe(
      RESERVED0 - QTY,
    );
    expect(
      postP.rows[0].stock,
      "stock FISICO deve permanecer STOCK0 (release nao baixa stock)",
    ).toBe(STOCK0);
    expect(Number.isInteger(postP.rows[0].stock)).toBe(true);
    expect(Number.isInteger(postP.rows[0].reserved)).toBe(true);
    // Flag do pedido virou (reserve-lifecycle): reserved=false; committed segue false
    // (cancelamento de pendente NUNCA passa por commit).
    expect(ord.rows[0].stock_reserved, "stockReserved deve virar false").toBe(false);
    expect(ord.rows[0].stock_committed, "stockCommitted deve continuar false").toBe(false);
