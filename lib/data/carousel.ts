 ...
 *
 * Regra: exibe os produtos MARCADOS (isCarousel), ativos e com estoque (>0), no
 ...
 * com estoque — assim a home nunca fica vazia logo apos o deploy, quando ainda
...
export function selectCarouselProducts(products: Product[], limit = CAROUSEL_LIMIT): Product[] {
  const eligible = products.filter((p) => p.isActive && p.stock > 0);
